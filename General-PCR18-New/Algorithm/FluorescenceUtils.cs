...
{
    public enum EDigitalFilterType
    {
        None = 0,
        Median = 1
    }

    public class PeakAmlCheckParam
    {
    ...
    public class TurbidityDyeConfig
    {
    ...
    /// MOT 校准
    /// </summary>
    public class MOTCalibration
    {
        private TurbidityConfig config;

        public MOTCalibration()
        {
        ...
        private void CalcFluDataByCtAndMOT(double[] inputY, double[] outputY, int startNo, int cycleCount, float adjustRatio)
        {
            ...
            for (int j = startNo + 1; j < cycleCount; j++)
            {
                ...
                {
                    // 将“下滑幅度”压缩至原来的 adjustRatio（0.1=抑制90%，0.2=抑制80%）
        ...
        public (double[] processedData, double ctValue) ProcessData(double[] famData, double[] motData, string channelName, double CT = 0)
        {
            ...
            // 根据通道从全局读取实际的 adjust scale（0.1/0.2）
            try
            {
                int idx = channelName == "FAM" ? 0 : channelName == "VIC" ? 1 : channelName == "ROX" ? 2 : channelName == "Cy5" ? 3 : 4;
                float userRatio = (float)Math.Max(0.0, Math.Min(1.0, Common.GlobalData.TurbidityAdjustScale[idx]));
                if (userRatio > 0) dyeConfig.Ratio = userRatio;
                LogHelper.Info("MOT adjust: channel={0}, ratio={1}", channelName, dyeConfig.Ratio);
            }
            catch { }
    ...
    public static class FluorescenceUtils
    {
        ...
        public static double FindCtPosWithParabola(int count, double[] x, double[] y, double[] firstDer, double[] secondDer, int threshold = 60)
        {
            ...
            do
            {
                ...
                {
                    double sum = 0;
                    for (int i = posMax - winWidth / 2; i < posMax + winWidth / 2 && i < count && i >= 0; ++i)
                        sum += zeroedSecond[i];
                    if (sum > threshold)
                    {
                        maxPosX += startPos;
        ...
        public static bool IsPeakAml(double[] pDX, double[] pDY, int nDataNum, PeakAmlCheckParam peakAmlChkParam, out double dOutCt, int threshold = 60)
        {
            ...
            double[] pDFirstDer = new double[nDataNum];
            ...
            CalcFirstDerivative(nDataNum, pDFiltered, pDFirstDer);//add by sunqp
            CalcSecondDerivative(nDataNum, pDFiltered, pDReturn);

            // pDFirstDer: edited by sunqp
            double dXPosMax = FindCtPosWithParabola(nDataNum, pDX, pDFiltered, pDFirstDer, pDReturn, threshold);
            int nxpos_max = (int)(dXPosMax + 0.5);

            bool bAble = false;
            if (nxpos_max > 3 && (dXPosMax < nDataNum) && pDReturn[nxpos_max - 1] > 2)
                if (nDataNum > 5)
                    bAble = true;
            if (!bAble)
                dXPosMax = 0;
            ...
            dOutCt = dXPosMax;
            double dmax = pDReturn[Math.Max(0, nxpos_max - 1)];
        ...
        )
        {
            ...
            hasCt = IsPeakAml(tempX, tempY, nCycleCount, peakParam, out dSndDrvCt, threshold);
            ...
            Console.WriteLine("Baseline out====================iCurStart=" + iCurStart + ",iCurEnd=" + iCurEnd);
        ...
        public static int CalcCycleOfMaxMOTRatioBy(double[] x, double[] y, double dMinRatio)
        {
            ...
            for (int i = 1; i < nCycleCount; ++i)
            {
                ...
                else if (y[i] < dMinValue) dMinValue = y[i];
            }
        ...
        public static void CalcAutoBaselinePosBySndDrvCt(double ct, int count, double[] x, double[] y, out int outStart, out int outEnd)
        {
            if (ct > 4)
            {
                ...
                if (outEnd - outStart > 4)
                {
                    ...
                    double[] tempX = new double[outEnd - outStart + 1];
                    double[] tempY = new double[outEnd - outStart + 1];
        ...
        public static void AdjustFluValueByBaseline(int count, double[] x, double[] y, int start, int end)
        {
            ...
            if (step >= 2)
            {
                ...
                if (Math.Abs(coef[1]) < 100 || (end - start > count / 3 || end - start > 10))
                {
                    for (int i = 0; i < count; ++i)
                    {
                        y[i] -= x[i] * coef[1];  // Subtract baseline trend
                        //Console.WriteLine(y[i] + "-=" + x[i] + "*" + coef[1]);
                    }
                }
            }
        }
        ...
        /// <returns></returns>
        public static double CalcBaselineAverage(int nPtCount, double[] pInputX, double[] pInputY, int iStart, int iEnd)
        {
        ...
        /// <summary>
        /// 计算基线均值, 这值用于计算归一化, DeltaRn, 原始扩增。这里需要使用第5步 smooth 的数据。
        /// </summary>
        /// <param name="pInput"></param>
        /// <param name="ct"></param>
        /// <returns></returns>
        public static double BaselineAverage(double[] pInput, double[] pOutput, double ct, out int iCurStart, out int iCurEnd)
        {
            ...
            Array.Copy(pInput, pDTempY, nCycleCount);
            ...
            if (nCycleCount > 3)
            {
                ...
                if (ct > 0)
                {
                    CalcAutoBaselinePosBySndDrvCt(ct, nCycleCount, pDTempX, pDTempY, out iCurStart, out iCurEnd);
                }
                else
                {
                    iCurStart = 1;
                    iCurEnd = nCycleCount;
                    CalcBaselinePosOfNoCt(pDTempX, pDTempY, out iCurStart, out iCurEnd);
                    if (nCycleCount > 20 && iCurEnd - iCurStart > nCycleCount * 2 / 3)
                    {
                        iCurStart = iCurEnd - nCycleCount * 2 / 3;
                    }
                ...
                double dAverage = CalcBaselineAverage(nCycleCount, pDTempX, pDTempY, iCurStart, iCurEnd);
                Array.Copy(pDTempY, pOutput, nCycleCount);
                return dAverage;
            }
